// filepath: /c:/Users/mdeleon/OneDrive - Entidad Controladora SA de CV/Documentos/Marcelo/impresion-3d-mty-page/src/modules/Quote/components/PrintQuote/FileInput/FileInput.jsx
...
const FileInput = () => {
  ...
  const [scalingItemCurrentHeight, setScalingItemCurrentHeight] =
    useState(null);
  const [scalingItemCurrentDepth, setScalingItemCurrentDepth] = useState(null);
  const [scalingItemCurrentAspectRatio, setScalingItemCurrentAspectRatio] =
    useState(null);
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFiles[0].size > 50000000) {
    ...
    selectedFiles.forEach((selectedFile) => {
      if (selectedFile.name.endsWith(".stl")) {
        ...
        loader.load(URL.createObjectURL(selectedFile), (geometry) => {
          ...
          const filesWithId = filesWithDimensions.map((file) => ({
            file,
            quantity: 1,
            id: nextId,
            aspectRatio: 100,
        ...
        selectedFile.name.endsWith(".STEP") ||
        selectedFile.name.endsWith(".step")
      ) {
        const fileWithDimensions = {
          file: selectedFile,
  ...
  const handleDrop = (e) => {
    ...
    droppedFiles.forEach((droppedFile) => {
      if (droppedFile.name.endsWith(".stl")) {
        const loader = new STLLoader();
      ...
      } else if (droppedFile.name.endsWith(".step")) {
        const fileWithDimensions = {
          file: droppedFile,
          type: "step",
        };
  ...
  const handleScalingChange = (value, index, type) => {
    ...
    switch (type) {
      case 1: // width
        foundObject.aspectRatio = parseFloat(
          ((value * 100) / scalingItem.file.original_width).toFixed(2)
        );
      ...
      case 2: // height
        foundObject.aspectRatio = parseFloat(
          ((value * 100) / scalingItem.file.original_height).toFixed(2)
        );
      ...
      case 3: // depth
        foundObject.aspectRatio = parseFloat(
          ((value * 100) / scalingItem.file.original_depth).toFixed(2)
        );
  ...
  const getAspectRatioValues = (foundObject) => {
    foundObject.file.width = parseFloat(
      (
        (foundObject.file.original_width * foundObject.aspectRatio) /
        100
      ).toFixed(2)
    );
    foundObject.file.height = parseFloat(
      (
        (foundObject.file.original_height * foundObject.aspectRatio) /
        100
      ).toFixed(2)
    );
    foundObject.file.depth = parseFloat(
      (
        (foundObject.file.original_depth * foundObject.aspectRatio) /
        100
      ).toFixed(2)
    );
  ...
  const deleteItem = (index) => {
    const updatedFiles = [...stlFiles];
    const indexToDelete = updatedFiles
      .flat()
      .findIndex((file) => file.id === index);
    ...
    <div className="file_input">
      <div className="file_input_container">
        <div className="file_input_presentation">
          <span className="file_input_presentation_title">
            {stlFiles.length} modelo(s) cargados
          </span>
          <span className="file_input_presentation_description">
            Ingresa tus archivos aquí para tener una referencia de tus
            necesidades, recuerda que todos estos archivos se trabajan de manera{" "}
            <strong>confidencial</strong>.
          </span>
          <span className="file_input_presentation_description">
            Aceptamos únicamente modelos .stl y .step de máximo 50mb, si tienes
            algún problema con el tamaño de tu archivo, puedes ponerte en
            contacto con nosotros mandando tus archivos a nuestro correo <strong>cotizaciones@3dprintmty.com</strong>
            .
          </span>
        </div>
        <div
                  ...
                  <div className="file_input_item_top_belt">
                    <div className="file_input_item_left">
                      <div className="file_input_item_counter">
                        Cantidad: &nbsp;
                        <button
                          onClick={() => handleDecrementQuantity(file.id)}
                        >
                          -
                        </button>
                        <input
                          ...
                          onChange={(e) =>
                            handleInputChange(e.target.value, file.id)
                          }
                          value={file.quantity}
                        />
                        <button
                          onClick={() => handleIncrementQuantity(file.id)}
                        >
                          +
                        </button>
                      </div>
                    ...
                    <div className="file_input_item_right">
                      ...
                      <Link
                        className="file_input_item_icon"
        ...
        <div className="overlay">
          <div className="file_input_overlay">
            <div className="file_input_overlay_container">
              <span className="file_input_overlay_title">
                Escalar modelo 3D
              </span>
              <div className="file_input_overlay_wrapper">
                <div className="file_input_overlay_item">
                  ...
                  <input
                    ...
                    value={scalingItemCurrentWidth}
                    onChange={(e) => setScalingItemCurrentWidth(e.target.value)}
                    onBlur={(e) =>
                      handleScalingChange(e.target.value, scalingItem.id, 1)
                    }
                  />
                  <span>mm</span>
                </div>
                <div className="file_input_overlay_item">
                  ...
                  <input
                    ...
                    value={scalingItemCurrentHeight}
                    onChange={(e) =>
                      setScalingItemCurrentHeight(e.target.value)
                    }
                    onBlur={(e) =>
                      handleScalingChange(e.target.value, scalingItem.id, 2)
                    }
                  />
                  <span>mm</span>
                </div>
                <div className="file_input_overlay_item">
                  ...
                  <input
                    ...
                    value={scalingItemCurrentDepth}
                    onChange={(e) => setScalingItemCurrentDepth(e.target.value)}
                    onBlur={(e) =>
                      handleScalingChange(e.target.value, scalingItem.id, 3)
                    }
                  />
                  <span>mm</span>
                </div>
                <div className="file_input_overlay_item">
                  <input
                    ...
                    value={scalingItemCurrentAspectRatio}
                    onChange={(e) =>
                      setScalingItemCurrentAspectRatio(e.target.value)
                    }
                    onBlur={(e) =>
                      handleScalingChange(e.target.value, scalingItem.id, 4)
                    }
                  />
                  <span>%</span>
                </div>
              ...
              <div className="file_input_overlay_buttons">
                <button
                  className="file_input_overlay_button"
                  onClick={() => resetScaling(scalingItem.id)}
                >
                  Reiniciar
                </button>
                <button
                  className="file_input_overlay_button"
                  id="file_input_overlay_button_send"
